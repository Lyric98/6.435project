...
def main(cfg):
    ...
    dataset = MyData(os.path.join(hydra.utils.get_original_cwd(), 'datasets/test1'))
    ...
    transition = Transition(32, 4, 32)
    theta_dim = sum([p.numel() for p in transition.parameters()])
    print("theta dim", theta_dim)

    U = cfg.U_scalar * torch.eye(xDIM)
    V = cfg.V_scalar * torch.eye(yDIM)
    mean_0 = torch.zeros(xDIM)
    cov_0 = torch.eye(xDIM)
    ...
    F_fn = F_Module(transition)
    G_fn = G_Module(decoder)
    p_0_dist = p_0_dist_module()
    ...
    def theta_func_constructor():
        ...
        return models.NN_Func_Polar(net, 32, theta_dim)
    ...
    def avg_f_prob(xs):
        ...
        for i in range(1, len(xs)):
            x_km1 = torch.from_numpy(xs[i-1]).reshape(1,xDIM)
            x_k = torch.from_numpy(xs[i]).reshape(1,xDIM)
